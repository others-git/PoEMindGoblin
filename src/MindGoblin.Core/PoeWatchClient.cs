...
public sealed class PoeWatchClient : IDisposable
{
    ...
    public PoeWatchClient(string userAgent, HttpClient? http = null)
    {
        ...
        // A plain client: poe.watch is unauthenticated, so none of the cookie handling
        // the trade site needed applies here. Decompression is on because the price
        // dumps are large and served gzipped.
        ...
        string category, string league, CancellationToken ct = default)
    {
        ...
        if (!resp.IsSuccessStatusCode)
            throw new PoeWatchException($"poe.watch returned {(int)resp.StatusCode} for {category}/{league}");

        var body = await resp.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
        using var doc = JsonDocument.Parse(body);
        if (doc.RootElement.ValueKind != JsonValueKind.Array)
            throw new PoeWatchException("poe.watch response was not an array");
...
/// <summary>poe.watch answered with something unusable.</summary>
public sealed class PoeWatchException(string message) : Exception(message);
